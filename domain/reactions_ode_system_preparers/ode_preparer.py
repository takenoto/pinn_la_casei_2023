    ...
    def prepare(self):
        def ode_system(x, y):
            ...
            for o in outputSimulationType.order:
                ...
                loss_o = lossPicker(loss_version=loss_version, o=o, args=args)
...
# ---------------------------------------------
def lossPicker(loss_version, o, args):
...
def lossV7(o, args):
    ...
    # TODO faz o mesmo esquema pra todos: valor mínimo, valor máximo, até pro volume
    # E punição por desvio do sinal
    # E XPS divide por volume com condicional ao invés de multiplicar
    # Minimum vale allowed
    N_min = -99
    # Maximum vale allowed
    N_max = +99
    ...
    if o in ["X", "P", "S"]:
        ...
        # The reactions for XPS
        N_dict = {
            # reaction, inlet conc, conc in the reactor
            "X": (dXdt, rX, inlet.X, X,),
            "P": (dPdt, rP, inlet.P, P),
            "S": (dSdt, rS, inlet.S, S),
        }
        dNdt, rN, inletN, concN = N_dict[o]
        
        # Se V for zero, usar um valor limite de 1e-7
        volume_threshold = tf.constant(1e-5)
        V_th = tf.where(tf.math.equal(V, 0), volume_threshold)
    ...
    loss = (1 + loss_multiplier) * (loss_derivative_abs + loss_minmax)
    # ----------------------
